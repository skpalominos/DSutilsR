...
psi_index <- function(cat_test,cat_train,base,var_cont,group_var){
  ...
  code <- function(cat_test,cat_train,base,var_cont,group_var){
    ...
    cut_point <- c(cut_point %>% quantile(probs = seq(0,1,0.1)) %>% as.vector())
    test <- (base %>% dplyr::filter(get(group_var)==cat_test) %>% select(var_cont))[,1]
    test <- table(cut(test,breaks=cut_point,include.lowest=T,labels=paste0("l",seq(1,10,1))))
    train <- (base %>% dplyr::filter(get(group_var)==cat_train) %>% select(var_cont))[,1]
    train <- table(cut(test,breaks=cut_point,include.lowest=T,labels=paste0("l",seq(1,10,1))))
    x <- log(test/train)
    x[x %in% c(Inf,-Inf)] <- 0
