...
colores_miss <- function(x){case_when(x < 25 ~ cell_spec(x,color="white",background="green",bold =T),
...
```{r ,echo=FALSE,warning=FALSE}
df_stats <- base %>% group_by(get(group_var)) %>% summarize_at(cont_vars,funs(mean(.,na.omit=TRUE) %>% round(3)))
...
```{r ,echo=FALSE,warning=FALSE}
df_stats <- base %>% group_by(get(group_var)) %>% summarize_at(cont_vars,funs(sd(.,na.rm=TRUE) %>% round(3)))
df_stats <- df_stats %>% setnames(old="get(group_var)",new=group_var)
```
...
```{r ,echo=FALSE,warning=FALSE}
df_stats <- base %>% group_by(get(group_var)) %>% summarize_at(cont_vars,funs(min(.,na.rm=TRUE) %>% round(3)))
df_stats <- df_stats %>% setnames(old="get(group_var)",new=group_var)
```
...
```{r ,echo=FALSE,warning=FALSE}
df_stats <- base %>% group_by(get(group_var)) %>% summarize_at(cont_vars,funs(max(.,na.rm=TRUE) %>% round(3)))
df_stats <- df_stats %>% setnames(old="get(group_var)",new=group_var)
```
